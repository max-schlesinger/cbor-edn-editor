...
suite("CBOR Parser Regression Tests", () => {
  ...
  validCases.forEach((input) => {
    test(`Sollte parsen: ${input}`, () => {
      ...
      if (result.lexErrors.length > 0) {
        console.error(`Error '${input}':`, result.lexErrors);
      }
      if (result.parseErrors.length > 0) {
        console.error(`Error '${input}':`, result.parseErrors);
      }
      assert.strictEqual(result.lexErrors.length, 0, `Error: ${input}`);
      assert.strictEqual(result.parseErrors.length, 0, `Error: ${input}`);
    });
  });
  ...
  invalidCases.forEach((input) => {
    test(`Error: ${input}`, () => {
      ...
      assert.strictEqual(hatFehler, true, `Error: ${input}`);
    });
  });
});
